...
implicit none
integer, parameter, private :: rk = kind(1.d0)
#if 1
real(rk) ::  LI=334.e3
#endif
private
...
real(rk)            :: KS    = 0.31      ! conductivity of snow - 0.31 W/(mK)
real(rk), parameter :: DS    = 330.0     ! density of snow - 330 kg/(m^3)
real(rk), parameter :: KI    = 2.03      ! conductivity of ice  - 2.03 W/(mK)
real(rk), parameter :: DI    = 905.0     ! density of ice  - 905 kg/(m^3)
real(rk), parameter :: CI    = 21e2      ! heat cap. of fresh ice - 2100 J/(kg K)
real(rk), parameter :: SI    = 1.0       ! salinity of sea ice
real(rk), parameter :: MU_TS = 0.054     ! relates freezing temp. to salinity
real(rk), parameter :: TFI   = -MU_TS*SI ! sea ice freezing temp. = -mu*salinity
real(rk), parameter :: CW    = 4.2e3     ! heat capacity of seawater
real(rk), parameter :: DW    = 1030.0    ! density of water for waterline - kg/(m^3)

! albedos are from CSIM4 assumming 0.53 visible and 0.47 near-ir insolation
real(rk)            :: ALB_SNO = 0.85       ! albedo of snow (not melting)
real(rk)            :: ALB_ICE = 0.5826     ! albedo of ice (not melting)
real(rk)            :: PEN_ICE = 0.3        ! ice surface penetrating solar fraction
real(rk)            :: OPT_DEP_ICE = 0.67   ! ice optical depth (m)
real(rk)            :: T_RANGE_MELT = 1.0   ! melt albedos scaled in below melting T

real(rk)            :: H_LO_LIM = 0.0       ! hi/hs lower limit for temp. calc.
...
real(rk), parameter :: ALB_OCEAN            = 0.10
...
logical, intent(in)   :: cm2_bugs_in
    real(rk), intent(in)   :: ks_in
    real(rk), intent(in)   :: h_lo_lim_in
...
real(rk), intent(  out) :: alb ! ice surface albedo (0-1)
real(rk), intent(  out) :: pen ! fraction of down solar penetrating the ice
real(rk), intent(  out) :: trn ! ratio of down solar at bottom to top of ice
real(rk), intent(in   ) :: hs  ! snow thickness (m-snow)
real(rk), intent(in   ) :: hi  ! ice thickness (m-ice)
real(rk), intent(in   ) :: ts  ! surface temperature
real(rk), intent(in   ) :: tfw ! seawater freezing temperature
real(rk) :: as, ai, cs
...
real(rk), intent(in   ) :: hs    ! snow thickness (m)
real(rk), intent(in   ) :: hi    ! ice thickness (m)
real(rk), intent(  out) :: ts    ! surface temperature (deg-C)
real(rk), intent(inout) :: t1    ! upper ice temperature (deg-C)
real(rk), intent(inout) :: t2    ! lower ice temperature (deg-C)
real(rk), intent(in   ) :: A     ! net surface heat flux (+ up) at ts=0 (W/m^2)
real(rk), intent(in   ) :: B     ! d(sfc heat flux)/d(ts) [W/(m^2 deg-C)]
real(rk), intent(in   ) :: I     ! solar absorbed by upper ice (W/m^2)
real(rk), intent(in   ) :: tfw   ! seawater freezing temperature (deg-C)
real(rk), intent(in   ) :: fb    ! heat flux from ocean to ice bottom (W/m^2)
real(rk), intent(in   ) :: dt    ! timestep (sec)
real(rk), intent(inout) :: tmelt ! accumulated top melting energy  (J/m^2)
real(rk), intent(inout) :: bmelt ! accumulated bottom melting energy (J/m^2)
!
...
subroutine thm_checkout(ts, hs, hi, t1, t2, bmelt, tmelt)
    real(rk), intent(in) :: ts, hs, hi, t1, t2, bmelt, tmelt
...
function e_to_melt(hs, h1, t1, h2, t2)
    real(rk), intent(in), optional :: hs, h1, t1, h2, t2
    real(rk)                       :: e_to_melt
...
subroutine add_to_top(h, t, h1, t1)
    real(rk),    intent(in) :: h, t
    real(rk), intent(inout) :: h1, t1
    real(rk)                :: f1
...
subroutine even_up(h1, t1, h2, t2)
    real(rk),intent(inout) :: h1, t1, h2, t2
    real(rk)               :: dh
...
subroutine ice3lay_resize(hs, hi, t1, t2, snow, frazil, evap, tmelt, bmelt, &
                          ...
                          snow_to_ice, bablt                                )
real(rk), intent(inout) :: hs          ! snow thickness (m-snow)
real(rk), intent(inout) :: hi          ! ice thickness (m-ice)
real(rk), intent(inout) :: t1          ! temperature of upper ice (deg-C)
real(rk), intent(inout) :: t2          ! temperature of lower ice (deg-C)
real(rk), intent(in   ) :: snow        ! new snow (kg/m^2-snow)
real(rk), intent(in   ) :: frazil      ! frazil in energy units
real(rk), intent(in   ) :: evap        ! ice evaporation (kg/m^2)
real(rk), intent(in   ) :: tmelt       ! top melting energy (J/m^2)
real(rk), intent(in   ) :: bmelt       ! bottom melting energy (J/m^2)
real(rk), intent(in   ) :: tfw         ! seawater freezing temperature (deg-C)
real(rk), intent(  out) :: heat_to_ocn ! energy left after ice all melted (J/m^2)
real(rk), intent(  out) :: h2o_to_ocn  ! liquid water flux to ocean (kg/m^2)
real(rk), intent(  out) :: h2o_from_ocn! evaporation flux from ocean (kg/m^2)
real(rk), intent(  out) :: snow_to_ice ! snow below waterline becomes ice
...
!KB    hs = hs - tmelt/e_to_melt(hs=1._rk)
    ...
    h2 = h2 - (tmelt-e_to_melt(hs,h1,t1))/e_to_melt(h2=1._rk,t2=t2)
    hs = 0.0
    h1 = 0.0
  else
...
real(rk), dimension(:,:,:), intent(inout) :: cn, hs, hi, t1, t2
